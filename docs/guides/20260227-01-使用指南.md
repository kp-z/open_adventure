...
下载文件：`open-adventure-v0.1.1-macos-arm64.tar.gz` (32MB)

### 2. 解压文件

```bash
tar -xzf open-adventure-v0.1.1-macos-arm64.tar.gz
cd release
```
...
**问题**：提示权限不足
```bash
permission denied: ./open-adventure
```
...
**解决**：删除数据库文件，应用会自动重建
```bash
rm open-adventure/open_adventure.db
./start.sh
```
...
```bash
cp open-adventure/open_adventure.db open_adventure_backup_$(date +%Y%m%d).db
```

### 恢复数据库

```bash
cp open_adventure_backup_20260227.db open-adventure/open_adventure.db
```
