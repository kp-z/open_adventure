...
### 二进制版本数据库问题
- **修复 Alembic 配置和迁移文件缺失问题**：修复了二进制版本启动时数据库表结构不完整的问题
  - 添加 `alembic.ini` 配置文件到打包列表
  - 添加 `alembic/env.py` 和 `script.py.mako` 到打包列表
  - 添加所有 `alembic/versions/*.py` 迁移文件到打包列表
  - 修复前：二进制版本使用 `create_all()` 创建表，缺少迁移中添加的字段（如 `process_pid`）
  - 修复后：二进制版本可以正常执行 Alembic 迁移，表结构完整
...
**注意**：v1.4.0 初始版本存在以下问题，已在后续提交中修复：
- Python 3.8 类型注解兼容性问题（已修复）
- 二进制版本 Alembic 配置和迁移文件缺失问题（已修复）
