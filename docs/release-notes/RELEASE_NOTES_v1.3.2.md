...
本次更新主要优化了多个核心页面在统一布局中的顶部留白表现，并发布 Linux x86_64 源码压缩包，便于在 Linux 环境中进行部署与验证。
...
本次按用户要求**优先发布 Linux 版本**，并采用**源码压缩包**方式提供发布产物：

- `open_adventure-v1.3.2-linux-x86_64-source.tar.gz`

说明：当前仅发布 Linux 源码包，未包含 macOS 对应发布物；如后续需要，可补充双平台发布。
