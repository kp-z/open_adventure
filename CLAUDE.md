...
### 服务启动规范
- **禁止自动启动前后端服务**：AI 助手不得自动执行 `./start.sh` 或任何启动服务的命令
- **必须获得用户明确同意**：在启动服务前，必须：
  1. 向用户说明将要启动的服务（前端/后端/Microverse）
  2. 询问用户是否同意启动
  3. 仅在用户明确同意后才执行启动命令
- **用户主动启动**：优先建议用户自己执行启动命令，而不是由 AI 代为执行
- **例外情况**：仅在以下情况可以自动启动：
  - 用户明确要求"启动服务"、"运行 start.sh"等指令
  - 用户在对话中已明确表示同意启动服务
