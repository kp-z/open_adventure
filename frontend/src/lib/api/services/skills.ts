...
export const skillsApi = {
  ...
  classify: (skillId: number) =>
    apiClient.post<{
      ...
      success: boolean;
      error?: string;
    }>('/skills/classify', { skill_id: skillId }),

  /**
   * 批量对 Skills 进行分类
   * @param skillIds 技能 ID 列表
   */
  classifyBatch: (skillIds: number[]) =>
    apiClient.post<{
      results: Array<{
        skill_id: number;
        skill_name: string;
      ...
      failed_count: number;
    }>('/skills/classify-batch', { skill_ids: skillIds }),
