...
export interface Skill {
  ...
  quality_grade?: 'A' | 'B' | 'C' | 'D' | 'F';  // 质量等级
  quality_evaluation?: QualityEvaluation;  // 详细评估结果
...
// Claude 配置的基础结构
export interface ClaudeConfigBase {
...
export interface CcSwitchProvider {
  ...
  config: ClaudeConfigBase;
}

// 完整的 Claude 配置响应（包含多个配置源）
export interface ClaudeSettings {
  active_source: 'cc-switch' | 'settings.json' | 'none';
  cc_switch: CcSwitchProvider | null;
  settings_json: ClaudeConfigBase | null;
  active_config: ClaudeConfigBase;
}
...
export interface Agent {
  ...
  meta: {
    ...
    exclude_skills?: string[];          // 排除的 skills 列表
    [key: string]: any;
  } | null;
...
// ============ Execution 相关类型 ============
export type ExecutionType = 'workflow' | 'agent_test' | 'agent_team' | 'terminal';
...
export interface DashboardStats {
  ...
  usage_source?: 'local_transcript' | 'fallback_meta';
