...
import { User, Clock, CheckCircle2, AlertCircle, Loader } from 'lucide-react';
...
function TaskItem({ task }: TaskItemProps) {
  ...
  const progress = totalDeps > 0 ? 50 : 0; // 简化计算
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div className={`flex items-center gap-1 text-xs px-2 py-1 rounded bg-${config.color}-500/20 text-${config.color}-400 shrink-0 ml-2`}>
          <Icon size={12} className={task.status === 'running' ? 'animate-spin' : ''} />
